...
namespace cv {
    ...
    // iGRAND RGB PROCESSING OPENCV CODE BEGIN
     ...
     */
    static void
    HarrisResponses(const Mat& img, const Mat& depth_img,
        ...
        const uchar* i_ptr00 = img.ptr<uchar>();
        const float* z_ptr00 = depth_img.ptr<float>();
        int istep = (int) (img.step / img.elemSize1());
        int zstep = (int) (depth_img.step / depth_img.elemSize1());
        int r = blockSize / 2;
        ...
        for (int i = 0; i < blockSize; i++)
            for (int j = 0; j < blockSize; j++)
                ofs[i * blockSize + j] = (int) (i * istep + j);
        ...
        for (ptidx = 0; ptidx < ptsize; ptidx++) {
            ...
            const uchar* i_ptr0 = i_ptr00 + (y0 - r + layerinfo[z].y) * istep + x0 - r + layerinfo[z].x;
            const float* z_ptr0 = z_ptr00 + (y0 - r + layerinfo[z].y) * zstep + x0 - r + layerinfo[z].x;
            int a = 0, b = 0, c = 0, d = 0, e = 0, f = 0;

            for (int k = 0; k < blockSize * blockSize; k++) {
                const uchar* i_ptr = i_ptr0 + ofs[k];
                const float* z_ptr = z_ptr0 + ofs[k];
                int Ix = (i_ptr[1] - i_ptr[-1])*2
                        + (i_ptr[-istep + 1] - i_ptr[-istep - 1])
                        + (i_ptr[istep + 1] - i_ptr[istep - 1]);
                int Iy = (i_ptr[istep] - i_ptr[-istep])*2
                        + (i_ptr[istep - 1] - i_ptr[-istep - 1])
                        + (i_ptr[istep + 1] - i_ptr[-istep + 1]);

                float Zx = (z_ptr[1] - z_ptr[-1])*2
                        + (z_ptr[-zstep + 1] - z_ptr[-zstep - 1])
                        + (z_ptr[zstep + 1] - z_ptr[zstep - 1]);
                float Zy = (z_ptr[zstep] - z_ptr[-zstep])*2
                        + (z_ptr[zstep - 1] - z_ptr[-zstep - 1])
                        + (z_ptr[zstep + 1] - z_ptr[-zstep + 1]);
                if (std::isnan(Zx) || std::isnan(Zy)) {
                    continue;
                }
            ...
            // Lambda to mix depth and intensity
            //float lambda = 255.f*100.f*4.f*255.f*100.f*4.f;
            float lambda = 255.f * 100.f * 4.f * 255.f * 100.f * 4.f;
            pts[ptidx].response = (((float) a * b - (float) c * c -
                    harris_k * ((float) a + b) * ((float) a + b))
                    + lambda * ((float) d * e - (float) f * f -
                    harris_k * ((float) d + e) * ((float) d + e))) * scale_sq_sq / 2;
        }
    }
    ...
    static void
    computeDescriptors_iGRAND(const Mat& imagePyramid, const Mat& depth_imagePyramid,
            const std::vector<Rect>& layerInfo, const std::vector<float>& layerScale,
            std::vector<KeyPoint>& keypoints,
            Mat& descriptors, const std::vector<Point>& _pattern, int dsize, int wta_k) {
        int istep = (int) imagePyramid.step;
        int zstep = (int) (depth_imagePyramid.step / depth_imagePyramid.elemSize1());
        ...
        for (j = 0; j < nkeypoints; j++) {
            ...
            const uchar* i_center = &imagePyramid.at<uchar>(cvRound(kpt.pt.y * scale) + layer.y,
                    cvRound(kpt.pt.x * scale) + layer.x);
            const float* z_center = &depth_imagePyramid.at<float>(cvRound(kpt.pt.y * scale) + layer.y,
                    cvRound(kpt.pt.x * scale) + layer.x);
            float x, y;
            int ix, iy;
            const Point* pattern = &_pattern[0];
            //            ushort* desc = descriptors.ptr<ushort>(j);
            uchar* desc = descriptors.ptr<uchar>(j);
            // area of triangles including 3 x,y locations in depth image
            // compare differences between center depth and pattern depths
#if 1
#define GET_VALUE(idx) \
               (x = pattern[idx].x*a - pattern[idx].y*b, \
                ...
                ix = cvRound(x), \
                iy = cvRound(y), \
                *(i_center + iy*istep + ix) )
#define GET_ZVALUE(idx) \
               (x = pattern[idx].x*a - pattern[idx].y*b, \
                y = pattern[idx].x*b + pattern[idx].y*a, \
                ix = cvRound(x), \
                iy = cvRound(y), \
                *(z_center + iy*zstep + ix) )
#else
#define GET_VALUE(idx) \
            (x = pattern[idx].x*a - pattern[idx].y*b, \
            y = pattern[idx].x*b + pattern[idx].y*a, \
            ...
            x -= ix, y -= iy, \
            cvRound(i_center[iy*istep + ix]*(1-x)*(1-y) + i_center[(iy+1)*istep + ix]*(1-x)*y + \
                    i_center[iy*istep + ix+1]*x*(1-y) + i_center[(iy+1)*istep + ix+1]*x*y))
#endif
...
#undef GET_VALUE
    ...
    class iGRAND_Impl : public iGRAND {
    public:

        explicit iGRAND_Impl(int _nfeatures, float _scaleFactor, int _nlevels, int _edgeThreshold,
        ...
        int getMaxFeatures() const override {
            return nfeatures;
        }

        void setScaleFactor(double scaleFactor_) override {
        ...
        void setNLevels(int nlevels_) override {
            nlevels = nlevels_;
        }
        ...
        void setEdgeThreshold(int edgeThreshold_) override {
            edgeThreshold = edgeThreshold_;
        }

        int getEdgeThreshold() const override {
            return edgeThreshold;
        }

        void setFirstLevel(int firstLevel_) override {
            firstLevel = firstLevel_;
        }

        int getFirstLevel() const override {
            return firstLevel;
        }

        void setWTA_K(int wta_k_) override {
            wta_k = wta_k_;
        }

        int getWTA_K() const override {
            return wta_k;
        }

        void setScoreType(int scoreType_) override {
            scoreType = scoreType_;
        }

        int getScoreType() const override {
            return scoreType;
        }

        void setPatchSize(int patchSize_) override {
            patchSize = patchSize_;
        }

        int getPatchSize() const override {
            return patchSize;
        }

        void setFastThreshold(int fastThreshold_) override {
            fastThreshold = fastThreshold_;
        }

        int getFastThreshold() const override {
        ...
        // sets the RGBD depth image adata
        void setDepthImage(Mat* img) override;
        // returns the RGBD depth image adata
        Mat* getDepthImage() const override;
    ...
    int iGRAND_Impl::descriptorSize() const {
        return kBytes;
    }

    int iGRAND_Impl::descriptorType() const {
        return CV_8U;
        //        return CV_16U;
    }

    int iGRAND_Impl::defaultNorm() const {
        return NORM_HAMMING;
    }

    void iGRAND_Impl::setDepthImage(Mat * imgData) {
        depth_image = imgData;
    }

    Mat * iGRAND_Impl::getDepthImage() const {
        return depth_image;
    }
     ...
     */
    static void computeKeyPoints_iGRAND(const Mat& imagePyramid,
            const UMat& uimagePyramid,
            const Mat& maskPyramid,
            const std::vector<Rect>& layerInfo,
            const UMat& ulayerInfo,
            const std::vector<float>& layerScale,
            const Mat& depth_imagePyramid,
            const UMat& depth_uimagePyramid,
            const Mat& depth_maskPyramid,
            const std::vector<Rect>& depth_layerInfo,
            const UMat& depth_ulayerInfo,
            const std::vector<float>& depth_layerScale,
        ...
        for (level = 0; level < nlevels; level++) {
            ...
            KeyPointsFilter::retainBest(keypoints, scoreType == iGRAND_Impl::HARRIS_SCORE ? 2 * featuresNum : featuresNum);
        ...
        if (scoreType == iGRAND_Impl::HARRIS_SCORE) {
            if (useOCL) {
            ...
            if (!useOCL)
                HarrisResponses(imagePyramid, depth_imagePyramid, layerInfo, allKeypoints, 7, HARRIS_K);
     ...
     */
    void iGRAND_Impl::detectAndCompute(InputArray _image, InputArray _mask,
            std::vector<KeyPoint>& keypoints,
        ...
        if ((!do_keypoints && !do_descriptors) || _image.empty() || !depth_image)
            return;
        ...
        Mat rgb_image = _image.getMat(), mask = _mask.getMat();
        if (rgb_image.type() != CV_8UC1)
            cvtColor(_image, rgb_image, COLOR_BGR2GRAY);
        ...
        Mat depth_imagePyramid;
        UMat depth_uimagePyramid;

        int level_dy = rgb_image.rows + border * 2;
        Point level_ofs(0, 0);
        Size bufSize((rgb_image.cols + border * 2 + 15) & -16, 0);

        for (level = 0; level < nLevels; level++) {
            float scale = getScale(level, firstLevel, scaleFactor);
            layerScale[level] = scale;
            Size sz(cvRound(rgb_image.cols / scale), cvRound(rgb_image.rows / scale));
        ...
        depth_imagePyramid.create(bufSize, CV_32F);
        //GaussianBlur(workingMat, workingMat, Size(7, 7), 2, 2, BORDER_REFLECT_101);
        if (!mask.empty())
            maskPyramid.create(bufSize, CV_8U);

        Mat prevImg = rgb_image, prevMask = mask;
        Mat prev_depth_Img = *depth_image;
        ...
        for (level = 0; level < nLevels; ++level) {
            ...
            Mat ext_depth_Img = depth_imagePyramid(wholeLinfo);
            Mat currImg = extImg(Rect(border, border, sz.width, sz.height)), currMask;
            Mat curr_depth_Img = ext_depth_Img(Rect(border, border, sz.width, sz.height));
            ...
            if (level != firstLevel) {
                ...
                resize(prev_depth_Img, curr_depth_Img, sz, 0, 0, INTER_LINEAR);
                if (!mask.empty()) {
                ...
                copyMakeBorder(curr_depth_Img, ext_depth_Img, border, border, border, border,
                        BORDER_REFLECT_101 + BORDER_ISOLATED);
                if (!mask.empty())
                    copyMakeBorder(currMask, extMask, border, border, border, border,
                        BORDER_CONSTANT + BORDER_ISOLATED);
            } else {
                copyMakeBorder(rgb_image, extImg, border, border, border, border,
                        BORDER_REFLECT_101);
                copyMakeBorder(*depth_image, ext_depth_Img, border, border, border, border,
                        BORDER_REFLECT_101);
                if (!mask.empty())
        ...
        if (do_keypoints) {
            ...
            computeKeyPoints_iGRAND(imagePyramid, uimagePyramid, maskPyramid,
                    layerInfo, ulayerInfo, layerScale,
                    depth_imagePyramid, depth_uimagePyramid,
                    maskPyramid,
                    layerInfo, ulayerInfo, layerScale,
                    keypoints,
                    nfeatures, scaleFactor, edgeThreshold, patchSize, scoreType,
                    useOCL, fastThreshold);
        } else {
            KeyPointsFilter::runByImageBorder(keypoints, rgb_image.size(), edgeThreshold);
        ...
        if (do_descriptors) {
            ...
            //            _descriptors.create(nkeypoints, dsize, CV_16U);
            ...
            if (!useOCL) {
                ...
                computeDescriptors_iGRAND(imagePyramid, depth_imagePyramid,
    ...
    Ptr<iGRAND> iGRAND::create(int nfeatures, float scaleFactor, int nlevels, int edgeThreshold,
            int firstLevel, int wta_k, int scoreType, int patchSize, int fastThreshold) {
        return makePtr<iGRAND_Impl>(nfeatures, scaleFactor, nlevels, edgeThreshold,
                firstLevel, wta_k, scoreType, patchSize, fastThreshold);
    }
    // iGRAND RGB PROCESSING OPENCV CODE END
}
